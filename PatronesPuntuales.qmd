...
## Pregunta de investigacíon: ¿Qué departamentos de Colombia requieren una mayor atención del Estado en términos de seguridad?
...
\begin{cases}
\begin{aligned}
\text{H}_0: &\text{Proceso Homogeneo} \\
\text{H}_1: &\text{Proceso Inhomogenea}
\end{aligned}
\end{cases}
$$
...
```{r, echo=FALSE}
load("Maps_Shapes_and_Objects/pcf_and_k.RData")
```


```{r, echo=FALSE}
plot(pcf_and_k[[1]], main = "Pair-Correlation estimada")
```
...
```{r, echo=FALSE}
plot(pcf_and_k[[2]], main = "Función K")
```
...
Se concluye que este estudio proporciona bases estadísticas que pueden contribuir a la formulación de políticas públicas orientadas a mejorar la seguridad en Colombia.
